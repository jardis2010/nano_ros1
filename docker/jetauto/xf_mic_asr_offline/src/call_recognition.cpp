...
int main(int argc, char *argv[])
{
	...
	string str3 = "休眠";				//语音识别相关字符串(voice recognition relative string)
	string str4 = "失败5次";				//语音识别相关字符串(voice recognition relative string)
	string str5 = "失败10次";				//语音识别相关字符串(voice recognition relative string)

	ros::init(argc, argv, "call_rec");    //初始化ROS节点(initialize ROS node)
	...
	while(ros::ok())
	{
		...
		{
			if(get_offline_recognise_result_client.call(GetOfflineResult_srv))    //请求离线命令词识别服务并返回应答为调用成功(request for offline voice command recognition service and return answer, call successfully.)
			{
				//ROS_INFO("succeed to call service \"get_offline_recognise_result_srv\"!");    //打印识别结果、置信度、识别命令词等信息(print recognition result, confidence, recognition voice command and other information)
